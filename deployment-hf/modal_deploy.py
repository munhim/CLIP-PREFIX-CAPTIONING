"""
Modal.com deployment for CLIP Prefix Captioning API
Deploys the model on a T4 GPU for inference

IMPORTANT: This file is meant to be deployed with Modal, not run directly.
Deploy with: modal deploy modal_deploy.py

DO NOT run: python modal_deploy.py (this will fail without dependencies)
"""

from __future__ import annotations  # Enable postponed evaluation of annotations

import modal
from modal import Image, asgi_app, gpu
import os
from typing import Optional, Tuple
from enum import Enum
from io import BytesIO

# Import standard library and Modal first
from fastapi import FastAPI, File, UploadFile, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import JSONResponse
from pydantic import BaseModel

# These imports will be available in Modal's container image
# Using try/except to allow Modal to parse if not installed locally
try:
    import torch
    ...
    # If missing locally, Modal will have them in the container
    # Create dummy classes for type checking if needed
    import sys
    if hasattr(sys, '_getframe') and 'modal' not in str(sys.modules.get('modal', '')):
        # We're in Modal's container - imports must work
        raise
    # Otherwise, we're parsing locally - these will be available in container
    pass
...
# Create Modal app
app = modal.App("clip-captioning-api")

# FastAPI app
fastapi_app = FastAPI(
    title="CLIP Prefix Captioning API (Modal)",
    description="Generate captions for images using CLIP + GPT-2",
    version="1.0.0"
)
...
# Model storage (will be initialized on first use)
models = {}
...
async def load_models_startup():
    ...
    try:
        ...
        if model_file is None:
            raise Exception(f"Could not find model file in HuggingFace repository: {HF_MODEL_ID}")
...
async def root():
    """API root endpoint"""
    return {
        "message": "CLIP Prefix Captioning API (Modal)",
        "version": "1.0.0",
        "model": HF_MODEL_ID,
        "endpoints": {
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "device": str(models.get('device', 'not_loaded')),
        "model_loaded": models.get('model') is not None,
        "clip_loaded": models.get('clip_model') is not None,
...
async def get_model_info():
    ...
    return {
        "caption_model": {
            "loaded": models.get('model') is not None,
            "hf_model_id": HF_MODEL_ID,
            "device": str(models.get('device', 'not_loaded'))
        },
        "clip_model": {
            "loaded": models.get('clip_model') is not None,
            "type": CLIP_MODEL_TYPE
        },
        "tokenizer": {
            "loaded": models.get('tokenizer') is not None
        }
    }
